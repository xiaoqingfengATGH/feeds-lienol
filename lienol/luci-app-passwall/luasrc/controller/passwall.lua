...
function index()
	...
	entry({"admin", "vpn", appname, "settings"}, cbi("passwall/global"), _("Basic Settings"), 1).dependent = true
	entry({"admin", "vpn", appname, "node_list"}, cbi("passwall/node_list"), _("Node List"), 2).dependent = true
	entry({"admin", "vpn", appname, "auto_switch"}, cbi("passwall/auto_switch"), _("Auto Switch"), 3).leaf = true
	entry({"admin", "vpn", appname, "other"}, cbi("passwall/other", {autoapply = true}), _("Other Settings"), 93).leaf = true
	if nixio.fs.access("/usr/sbin/haproxy") then
		entry({"admin", "vpn", appname, "haproxy"}, cbi("passwall/haproxy"), _("Load Balancing"), 94).leaf = true
	end
	entry({"admin", "vpn", appname, "node_subscribe"}, cbi("passwall/node_subscribe"), _("Node Subscribe"), 95).dependent = true
	entry({"admin", "vpn", appname, "rule"}, cbi("passwall/rule"), _("Rule"), 96).leaf = true
	entry({"admin", "vpn", appname, "app_update"}, cbi("passwall/app_update"), _("App Update"), 97).leaf = true
	entry({"admin", "vpn", appname, "node_config"}, cbi("passwall/node_config")).leaf = true
	entry({"admin", "vpn", appname, "shunt_rules"}, cbi("passwall/shunt_rules")).leaf = true
	entry({"admin", "vpn", appname, "acl"}, cbi("passwall/acl"), _("Access control"), 98).leaf = true
	entry({"admin", "vpn", appname, "log"}, form("passwall/log"), _("Watch Logs"), 999).leaf = true
	entry({"admin", "vpn", appname, "server"}, cbi("passwall/server/index"), _("Server-Side"), 99).leaf = true
	entry({"admin", "vpn", appname, "server_user"}, cbi("passwall/server/user")).leaf = true
	...
	entry({"admin", "vpn", appname, "luci_check"}, call("luci_check")).leaf = true
	entry({"admin", "vpn", appname, "luci_update"}, call("luci_update")).leaf = true

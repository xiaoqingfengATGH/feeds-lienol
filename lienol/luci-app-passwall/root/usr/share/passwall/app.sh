...
start_dns() {
	...
	homelede)
		...
		chnlist_param="${TMP_PATH}/chnlist"
		[ "$(config_t_get global fair_mode 1)" = "1" ] && extra_mode="-f"
		ln_start_bin "$(first_type chinadns-ng)" chinadns-ng -l "${DNS_PORT}" -c "127.0.0.1#6053" ${chnlist_param:+-m "${chnlist_param}" -M} -t "127.0.0.1#5053" ${gfwlist_param:+-g "${gfwlist_param}"} $extra_mode
		echolog "  + 过滤服务：HomeLede内置(ChinaDNS-NG:${DNS_PORT}${extra_mode})：国内DNS：127.0.0.1#6053，海外DNS：127.0.0.1#7053。 ${msg}"
	;;
	*)
